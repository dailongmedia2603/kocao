...
serve(async (req) => {
  ...
  try {
    ...
    if (apiKeyError || !apiKeyData) {
      throw new Error("Chưa có API Key Gemini nào được cấu hình trong hệ thống.");
    }
    ...
    if (!geminiResponse.ok || !geminiData.candidates || geminiData.candidates.length === 0) {
      if (geminiData?.promptFeedback?.blockReason) {
        throw new Error(`Nội dung bị chặn vì lý do an toàn: ${geminiData.promptFeedback.blockReason}.`);
      }
