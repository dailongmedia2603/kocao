...
const ApiKeyRow = ({ apiKey }: { apiKey: ApiKey }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (keyId: string) => {
      ...
      const { error } = await supabase.from("user_voice_api_keys").delete().eq("id", keyId);
  ...
  const checkConnectionMutation = useMutation({
    mutationFn: async (key: string) => {
      const { data, error } = await supabase.functions.invoke("check-voice-api-key", { body: { apiKey: key } });
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Cấu hình API GenAIPro Voice</CardTitle>
            <CardDescription>
              Thêm và quản lý các API Key GenAIPro Voice của bạn.
              Bạn có thể lấy key từ <a href="https://genaipro.vn/" target="_blank" rel="noopener noreferrer" className="text-primary underline">GenAIPro</a>.
            </CardDescription>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-10 border-2 border-dashed rounded-lg">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Bấm "Thêm Key" để thêm API Key GenAIPro Voice đầu tiên của bạn.
              </p>
            </div>
          )}
        </CardContent>
