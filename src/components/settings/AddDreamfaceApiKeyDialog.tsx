...
export const AddDreamfaceApiKeyDialog = ({ isOpen, onOpenChange }: AddDreamfaceApiKeyDialogProps) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: { account_id: "", user_id_dreamface: "", token_id: "", client_id: "" },
  });

  const addKeyMutation = useMutation({
    mutationFn: async (values: z.infer<typeof formSchema>) => {
      await api.settings.saveDreamface({
    ...
    onSuccess: () => {
      showSuccess("Cập nhật Credentials thành công!");
      queryClient.invalidateQueries({ queryKey: ["dreamface_api_status", user?.id] });
      onOpenChange(false);
      form.reset();
    },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Cấu hình Credentials Tạo Video</DialogTitle>
          <DialogDescription>Nhập các thông tin xác thực để kết nối với dịch vụ tạo video. Các thông tin cũ sẽ bị ghi đè.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
