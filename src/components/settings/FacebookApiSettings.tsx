...
const FacebookTokenRow = ({ token }: { token: FacebookToken }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (tokenId: string) => {
      ...
      const { error } = await supabase.from("user_facebook_tokens").delete().eq("id", tokenId);
  ...
  const checkConnectionMutation = useMutation({
    mutationFn: async (accessToken: string) => {
      const { data, error } = await supabase.functions.invoke("check-facebook-token", { body: { accessToken } });
