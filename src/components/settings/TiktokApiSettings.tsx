...
const TiktokTokenRow = ({ token }: { token: TiktokToken }) => {
...
const TiktokApiSettings = () => {
  ...
  const { data: tiktokTokens, isLoading } = useQuery({
    queryKey: ["tiktok_tokens", user?.id],
    queryFn: () => fetchTiktokTokens(user!.id),
    enabled: !!user,
  });
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Cấu hình API TikTok</CardTitle>
            <CardDescription>
              Thêm và quản lý các Access Token để kết nối với TikTok qua proxy.
            </CardDescription>
          </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="space-y-4">
              {tiktokTokens.map((token) => (
                <TiktokTokenRow key={token.id} token={token} />
              ))}
            </div>
          ) : (
            <div className="text-center py-10 border-2 border-dashed rounded-lg">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Bấm "Thêm Token" để thêm Access Token TikTok đầu tiên của bạn.
              </p>
            </div>
          )}
        </CardContent>
      </Card>
      <AddTiktokTokenDialog isOpen={isAddDialogOpen} onOpenChange={setAddDialogOpen} />
    </>
  );
};
