...
const TiktokApiSettings = () => {
  ...
  const { data: status, isLoading, refetch } = useQuery({
    queryKey: ["tiktok_status", user?.id],
    queryFn: () => api.settings.getTiktok(),
    enabled: !!user,
  });

  const deleteMutation = useMutation({
    mutationFn: api.settings.deleteTiktok,
    onSuccess: () => {
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Thêm và quản lý Access Token để kết nối với TikTok qua proxy.
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent>
            ...
            <div className="flex items-center justify-between gap-4 p-4 border rounded-lg bg-background/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{status?.has_token ? "Hệ thống đã kết nối với TikTok." : "Vui lòng thêm Access Token để kết nối."}</p>
                </div>
              </div>
