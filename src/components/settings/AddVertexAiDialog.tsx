...
export const AddVertexAiDialog = ({ isOpen, onOpenChange }: AddVertexAiDialogProps) => {
  ...
  const addKeyMutation = useMutation({
    mutationFn: async (values: z.infer<typeof formSchema>) => {
      await api.settings.saveVertexAi(JSON.parse(values.credentials));
    },
    onSuccess: () => {
      showSuccess("Cập nhật thông tin xác thực Vertex AI thành công!");
      queryClient.invalidateQueries({ queryKey: ["vertex_ai_status", user?.id] });
      onOpenChange(false);
      form.reset();
    },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Cấu hình Vertex AI</DialogTitle>
          <DialogDescription>Dán nội dung file JSON của Service Account từ Google Cloud. Thông tin cũ sẽ bị ghi đè.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={addKeyMutation.isPending}>
                {addKeyMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Đang lưu...</> : "Lưu Cấu Hình"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
