...
export const AddFacebookTokenDialog = ({ isOpen, onOpenChange }: AddFacebookTokenDialogProps) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: { access_token: "", check_url: "https://api.akng.io.vn/graph/me" },
  });

  const addTokenMutation = useMutation({
    mutationFn: async (values: z.infer<typeof formSchema>) => {
      await api.settings.saveFacebook(values.access_token, values.check_url);
    },
    onSuccess: () => {
      showSuccess("Cập nhật Access Token thành công!");
      queryClient.invalidateQueries({ queryKey: ["facebook_status", user?.id] });
      onOpenChange(false);
      form.reset();
    },
    onError: (error: Error) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Cấu hình Access Token Facebook</DialogTitle>
          <DialogDescription>
            Nhập Access Token để kết nối với Facebook. Token cũ sẽ bị ghi đè.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
