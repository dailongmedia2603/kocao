...
// Icons
import { Plus, MoreHorizontal, Trash2, Loader2, Shield, User as UserIcon } from "lucide-react";
...
const RoleBadge = ({ role }: { role: UserProfile['role'] }) => {
  if (role === 'admin') {
    return <Badge variant="destructive" className="items-center gap-1"><Shield className="h-3 w-3" /> Admin</Badge>;
  }
  return <Badge variant="secondary" className="items-center gap-1"><UserIcon className="h-3 w-3" /> User</Badge>;
};
...
const StatusBadge = ({ status }: { status: UserProfile['status'] }) => {
  switch (status) {
    ...
    case 'banned':
      return <Badge variant="destructive">Cấm</Badge>;
    default:
      ...
      <div className="p-6 lg:p-8">
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Thêm, chỉnh sửa và quản lý tất cả người dùng trong hệ thống.</p>
          </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Hành động này không thể hoàn tác. Người dùng "{`${userToDelete?.last_name || ''} ${userToDelete?.first_name || ''}`.trim()}" sẽ bị xóa vĩnh viễn.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
