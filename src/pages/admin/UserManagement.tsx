...
// Icons
import { Plus, MoreHorizontal, User, Shield, Trash2, Loader2 } from "lucide-react";
...
const statusMap = {
  ...
  banned: { text: "Cấm", className: "bg-red-100 text-red-800" },
};
                ...
                {isLoading ? (
                  ...
                  users.map((user) => (
                    <TableRow key={user.id}>
                      <TableCell className="font-medium">{`${user.last_name || ''} ${user.first_name || ''}`.trim()}</TableCell>
                      <TableCell>{user.email}</TableCell>
                      <TableCell>
                        <Badge variant="outline" className={user.role === 'admin' ? "border-red-200 text-red-800" : ""}>
      ...
      <AlertDialog open={!!userToDelete} onOpenChange={() => setUserToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Hành động này không thể hoàn tác. Người dùng "{userToDelete?.first_name}" sẽ bị xóa vĩnh viễn.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
